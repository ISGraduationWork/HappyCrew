...
def login():
    if request.method == 'POST':
        ...
        cmd = "SELECT user_id, user_name, password FROM users WHERE user_name = ?"
...
def home():
    ...
    cmd = f"SELECT * FROM class_info WHERE user_id = {user_id} ORDER BY position"
    class_info = sql(cmd)
    print(class_info)
...
def create_class():
    if request.method == 'POST':
        ...
        # 現在の最大positionを取得
        cmd = f"SELECT MAX(position) FROM class_info WHERE user_id = {user_id}"
        result = sql(cmd)
...
def update_class_order():
    try:
        ...
        for index, class_id in enumerate(order):
        	cmd = f"UPDATE class_info SET position = {index} WHERE class_id = {class_id}"
            queries.append(cmd)
...
def update_class_order_in_db(class_id, position):
    cmd = f"UPDATE class_info SET position = {position} WHERE class_id = {class_id}"
    sql(cmd)
...
def class_detail(class_id):
    user_id = current_user.id
    cmd = f"SELECT * FROM class_info WHERE class_id = {class_id} AND user_id = {user_id}"
    class_info = sql(cmd)
    ...
    cmd = f"""
        SELECT class_detail_id, class_minutes, neutral, happy, sad, surprised, anger, count, class_score
        FROM class_detail
    ...
    return render_template('class_detail.html', class_info=class_info[0], class_result=class_result)
